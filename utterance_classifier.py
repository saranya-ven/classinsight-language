...
def get_f1_auc_plots_through_datasizes(x,y,classifier,test_val_size=0.10,rn_state=1):
    ...
    for i in range(1,100):
        ...
        best_th_val,precrec_aucscore_val=report_precision_recall_fscore(yval, y_pred_proba_val,variable_name,use_plots=False)
    ...
    plot_metric_by_data_size("f1_"+variable_name, all_f1_scores_percs, range(1,100))
    plot_metric_by_data_size("auc_"+variable_name, all_auc_scores_percs, range(1,100))
  
        
if __name__ == "__main__":
    
    dataset_path="transcripts/official_transcripts/4_Datasets/dataset_all_20dim.csv"
    ...
    for var_index in interest_variables:
        ...
        #if we want to plot performance vs datasize, the following 2 lines replaces the rest
        #print("processing ",variable_name)
        #get_f1_auc_plots_through_datasizes(x, y, classifier, test_val_size=0.10, rn_state=1)
        xtrain,xval,xtest,ytrain,yval,ytest=train_test_validation_split(x, y, testval_size=0.10, rm_state=1)
        ...
        #auc_score=report_precision_recall_auc(yval, y_pred_proba_val,variable_name)
        best_th_val,precrec_aucscore_val=report_precision_recall_fscore(yval, y_pred_proba_val,variable_name)#get best th on validation
        y_test_bestpred = [True if prob>best_th_val else False for prob in y_pred_proba_test]
        best_f1_score_test=f1_score(ytest,y_test_bestpred)
